...
import { API_PATHS, productPath } from '../../config/api.js';
...
function AdminPanel() {
  ...
  const loadProducts = () => {
    setLoading(true);
    api
      .get(`${API_PATHS.products}/admin/all`)
  ...
  const toggleActive = async (product) => {
    try {
      await api.put(productPath(product.id), { is_active: !product.is_active });
      loadProducts();
    } catch (err) {
      console.error(err);
  ...
  const deleteProduct = async (product) => {
    ...
    try {
      await api.delete(productPath(product.id));
      loadProducts();
    } catch (err) {
      console.error(err);
